...
exports.grantAcess = catchAsyncHandler(async (req, res, next) => {
  ...
  if (!token) {
    return next(new ErrHandlingClass('Please log in', 401));
  }
  ...
  if (!refreshUserStat) {
    return next(new ErrHandlingClass('This user/token is no longer exist', 401));
  }
  ...
  if (refreshUserStat.changedPasswordCheck(decodedToken.iat)) {
    return next(new ErrHandlingClass('the password is already changed', 401));
  }
...
exports.isLoggedIn = catchAsyncHandler(async (req, res, next) => {
  if (req.cookies.jwt) {
    try {
      // check whether a token is valid or not (whether the token signature is valid or not)
      const decodedToken = await promisify(jwt.verify)(req.cookies.jwt, process.env.SECRET)
...
exports.authorization = (...roles) => (req, res, next) => {
  // "roles" is an array of authorize users with specific role to perform some actions, 
  // OR to enable certain access to certain routes (for now just for admin)
...
exports.forgotPassword = catchAsyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    // Sending email to user with a random token
    ...
    await user.save({ validateBeforeSave: false });
    return next( new ErrHandlingClass('We detect an error occurs while the password reset process, please try again', 500) )
  }
});
...
exports.resetPassword = catchAsyncHandler(async (req, res, next) => {
  ...
  // Check whether a token was expires or not
  if (!user) {
    return next( new ErrHandlingClass('Invalid token or token has expired, please try again', 400) )
  }
	
  // Update password and delete passwordResetToken and passwordResetExpires
  ...
  // Update the "passwordChangedAt" date and Log the user in and send a JWT
  genResJWT(user, 200, req, res);
});
