...
const sendingEmail = require('../utilities/nodemail');
...
exports.forgotPassword = catchAsyncHandler(async (req, res, next) => {
	...
	const message = `That's ok, it happens! Let's reset your password click the link below: ${passwordResetURL}`;

	try {
		await sendingEmail({
			email: user.email,
			subject: 'Your password reset token is valid for 5 min',
			message
		});
